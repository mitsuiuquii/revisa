...
export default function Leaderboard() {
  ...
  return (
    <Layout>
      ...
      <p className="text-slate-600 font-bold mt-1">Top alunos por XP</p>

      <div className="space-y-2 mt-6">
        {list.map((u, i) => {
          const TierIcon = Icons[u.tier?.icon] || Icons.Medal;
          return (
            <motion.div key={u.id}
            ...
            >
              <div className="w-9 h-9 rounded-xl flex items-center justify-center border-2 border-slate-900 font-display font-extrabold shrink-0"
                style={{ background: medalColor(u.rank_position), color: u.rank_position <= 3 ? "white" : "#0F172A" }}>
                {u.rank_position <= 3 ? <Crown className="w-5 h-5" strokeWidth={3} /> : u.rank_position}
              </div>
              <div className="w-9 h-9 rounded-full flex items-center justify-center font-display font-extrabold text-white shrink-0"
